...
namespace ASTImpl {
    ...
    public:
        ...
        virtual double Evaluate(const std::function<double(Position)>& cell_lookup) const = 0;
    ...
    namespace {
        ...
        public:
            ...
            double Evaluate(const std::function<double(Position)>& cell_lookup) const override {
                double lhs_val = lhs_->Evaluate(cell_lookup);
                double rhs_val = rhs_->Evaluate(cell_lookup);
                double result = 0.0;
        ...
        public:
            ...
            double Evaluate(const std::function<double(Position)>& cell_lookup) const override {
                double value = operand_->Evaluate(cell_lookup);
                double result = 0.0;
...
double FormulaAST::Execute(const std::function<double(Position)>& cell_lookup) const {
